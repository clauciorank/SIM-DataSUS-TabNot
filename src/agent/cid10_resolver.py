...
def get_causas_contexto_for_plan(pergunta: str) -> str:
    """
    Ponto de entrada para o grafo: extrai termo de causa, busca no depara, formata para SQL.
    Se não houver termo ou busca vazia, retorna "" (o plan usará fallback get_cause_context_for_plan).
    Para termos amplos (ex.: doença cardiovascular), prefere filtro por capítulo CID.
    """
    termo = extract_cause_phrase_heuristic(pergunta)
    if not termo:
        return ""
    termo = termo.strip().rstrip("?.,;:!").strip()
    if not termo:
        return ""
    low_pergunta = pergunta.lower()
    termo_low = termo.lower().strip()
    # Termos específicos (ex.: neoplasia óssea): usar busca por códigos estrita, não capítulo inteiro
    if termo_low in CHAPTER_OR_STRICT_TERMS or any(t in termo_low for t in ("neoplasia óssea", "neoplasia ossea", "neoplasia do osso")):
        use_chapter_first = False
    else:
        # Preferir capítulo quando: pergunta menciona "capítulo" OU termo é amplo (cardiovascular, etc.)
        use_chapter_first = (
            "capítulo" in low_pergunta or "capitulo" in low_pergunta
            or _is_chapter_like_term(termo)
        )
    if use_chapter_first:
        chapter_matches = search_cid10_chapters(termo, limit=3)
        if chapter_matches:
            candidates_chapter = [{"capitulo_descricao": c} for c in chapter_matches]
            sql_part = format_causas_for_sql(candidates_chapter, by_chapter=True)
            if sql_part:
                return (
                    "Filtro de causa/doença (use EXATAMENTE na cláusula WHERE): "
                    + sql_part
                    + ". Não use ILIKE em texto livre nem invente códigos."
                )
    code_matches = search_cid10(termo, limit=25, min_score=55)
    if code_matches:
        sql_part = format_causas_for_sql(code_matches, by_chapter=False)
        if sql_part:
            return (
                "Filtro de causa/doença (use EXATAMENTE na cláusula WHERE): "
                + sql_part
